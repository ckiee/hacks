let cycle = `LLLLLLLLL.LLLLL.LLLLL.LLLL.LLLLLL.LL.LLLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLLLLL.LLLL.LLLL.LLLL.LLLLL
...
while (streak>10) {
	...
	let cycleViz = cycle.map(x => x.join("")).join("\n");
	// console.log(cycle.map(x => x.join("")).join("\n"), "\n------");
	if (lastCycleViz == cycleViz) {
		streak++;
	} else {
		streak = 0;
	}
	lastCycleViz = cycle.map(x => x.join("")).join("\n");
}
console.log(
