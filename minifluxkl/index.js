document.addEventListener("DOMContentLoaded", () => {
    ...
    if (location.pathname.match(/^\/(history|unread|feed\/\d+\/entries)$/)) {
        const ZOOM = 1.25;
        const USE_EXT = true;
...
`);
        ...
        [iframe, bgIframe].map(fr => fr.addEventListener("load", () => fr.setAttribute("data-loaded", true)))
        ...
        let focused;
        const viewUrlOf = a => {
            const ext = a.querySelector(".item-meta-icons-external-url a").getAttribute("href");
            const int = a.querySelector(".item-title a").getAttribute("href");
            let useExt = USE_EXT;
            if (new URL(ext).hostname == "cloudwithlightning.net") useExt = false;
        ...
        articles.map(article =>
            article.onfocus = e => {
                if (focused !== article) {
                    ...
                    let ofs = idx == 0 ? 0 : -articles[idx - 1].offsetHeight;
                    if (article !== articles[0]) {
                        article.scrollIntoView(true)
                        window.scrollBy({ top: ofs });
                    }
                    preview.style.transform = `translateY(${article.offsetTop - preview.offsetTop + ofs}px)`;
                    ...
                    bgIframe.src = viewUrlOf(articles[articles.indexOf(article) + 1]);
                }
                focused = article;
            }
        )
